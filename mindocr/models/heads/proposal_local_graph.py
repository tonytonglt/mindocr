...
class ProposalLocalGraphs:
    ...
    def generate_local_graphs(self, sorted_dist_inds, node_feats):
        ...
        for graph_ind, pivot_knn in enumerate(pivot_knns):
            ...
            knn_inds = ops.cast(
                ms.Tensor([node2ind_map[i]
                                  for i in pivot_knn[1:]]), 'int64')
            pivot_feats = node_feats[pivot_ind]
            normalized_feats = node_feats[ms.Tensor(
            ...
            pad_adjacent_matrix = ops.zeros((num_max_nodes, num_max_nodes), )
            pad_adjacent_matrix[:num_nodes, :num_nodes] = ops.cast(
                ms.Tensor(adjacent_matrix), 'float32')

            pad_normalized_feats = ops.concat(
                [
                    normalized_feats, ops.zeros(
                        (num_max_nodes - num_nodes, normalized_feats.shape[1]),
                    )
                ],
                axis=0)

            local_graph_nodes = ms.Tensor(pivot_local_graph)
            local_graph_nodes = ops.concat(
                [
                    local_graph_nodes, ops.zeros(
        ...
        local_graphs_node_feat = ops.stack(local_graphs_node_feat, 0)
        adjacent_matrices = ops.stack(adjacent_matrices, 0)
        pivots_knn_inds = ops.stack(pivots_knn_inds, 0)
        pivots_local_graphs = ops.stack(pivots_local_graphs, 0)
    ...
    def __call__(self, preds, feat_maps):
        ...
        if preds.ndim == 4:
            ...
            preds = ops.squeeze(preds)
        pred_text_region = ops.sigmoid(preds[0]).asnumpy()
        pred_center_region = ops.sigmoid(preds[1]).asnumpy()
        pred_sin_map = preds[2].asnumpy()
