...
class LocalGraphs:
    ...
    def generate_gcn_input(self, node_feat_batch, node_label_batch,
        ...
        for batch_ind, sorted_dist_inds in enumerate(sorted_dist_ind_batch):
            ...
            for graph_ind, pivot_knn in enumerate(pivot_knns):
                ...
                pad_adjacent_matrix = ops.zeros(
                    (num_max_nodes, num_max_nodes))
                pad_adjacent_matrix[:num_nodes, :num_nodes] = ops.cast(
                    ms.Tensor(adjacent_matrix), ms.float32)

                if num_max_nodes != num_nodes:
        ...
        pivots_gt_linkage = ops.stack(pivots_gt_linkage, 0)
    ...
    def __call__(self, feat_maps, comp_attribs):
        ...
        assert isinstance(feat_maps, ms.Tensor)
