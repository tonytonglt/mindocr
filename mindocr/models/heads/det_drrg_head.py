...
class DRRGHead(nn.Cell):
    ...
    def construct(self, inputs, targets=None):
        ...
        if self.training:
            ...
            gt_comp_attribs = targets
            print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!', gt_comp_attribs.shape)
    ...
    def single_test(self, feat_maps):
        ...
        feat_maps = ops.concat((feat_maps, pred_maps), axis=1)
        print(000)
        return_value = self.graph_test(pred_maps, feat_maps)
        print(len(return_value))
        none_flag, graph_data = return_value
        ...
        gcn_pred = self.gcn(local_graphs_node_feat, adjacent_matrices,
                            pivots_knn_inds)
        pred_labels = ops.softmax(gcn_pred, axis=1)
